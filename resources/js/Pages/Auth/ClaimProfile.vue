        ...
        <div class="max-w-lg mx-auto">
            ...
            <div v-if="step === 'search'" class="space-y-4">
                ...
                <div>
                    ...
                    <select v-model="selectedSet" class="w-full border-gray-300 rounded-md shadow-sm focus:border-amber-500 focus:ring-amber-500">
                        <option value="">All Sets</option>
                        <option v-for="s in sets" :key="s.id" :value="s.id">{{ s.name }}</option>
                    </select>
                </div>
                <button @click="searchProfiles"
                    class="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-2 px-4 rounded">
                    Search
                </button>
                ...
                <div v-if="searchResults.length > 0" class="mt-6 space-y-3">
                    ...
                    <div v-for="result in searchResults" :key="result.id"
                        ...
                        class="border rounded-lg p-4 cursor-pointer hover:border-amber-500 hover:bg-amber-50 transition">
                ...
                <div v-if="searchResults.length === 0 && searchQuery.length > 2" class="mt-4 text-center text-gray-500">
                    No profiles found. <Link :href="route('register')" class="text-amber-600 hover:text-amber-700">Register as a new member</Link>
                </div>
            </div>
                ...
                <div v-else class="space-y-4">
                    ...
                    <div>
                        ...
                        <input v-model="otp" type="text" maxlength="6" placeholder="Enter 6-digit code"
                            class="w-full border-gray-300 rounded-md shadow-sm focus:border-amber-500 focus:ring-amber-500 text-center text-2xl tracking-widest" />
            ...
            <div v-if="step === 'complete'" class="text-center py-8">
                ...
                <Link :href="route('dashboard')"
                    class="bg-amber-500 hover:bg-amber-600 text-white font-bold py-2 px-6 rounded inline-block">
                    Go to Dashboard
                </Link>
            </div>
        </div>
