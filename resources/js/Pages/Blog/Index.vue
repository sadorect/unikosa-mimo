    ...
    <AuthLayout :auth="$page.props.auth" :settings="$page.props.settings">
        ...
        <template #header>
            <div class="flex justify-between items-center">
                ...
                <Link :href="route('blog.create')" class="bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium px-4 py-2 rounded">Write Post</Link>
            </div>
        </template>
        <div class="py-12">
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4">
                            ...
                            <div class="space-y-1">
                                <button @click="filterCategory(null)"
                                    ...
                                    :class="!activeCategory ? 'bg-amber-100 text-amber-800' : 'hover:bg-gray-100 dark:hover:bg-gray-700'">
                                    All Posts
                                </button>
                                <button v-for="cat in categories" :key="cat.id" @click="filterCategory(cat.id)"
                                    class="block w-full text-left px-3 py-2 rounded text-sm transition"
                                    :class="activeCategory == cat.id ? 'bg-amber-100 text-amber-800' : 'hover:bg-gray-100 dark:hover:bg-gray-700'">
                                    {{ cat.name }}
