    ...
    <AuthLayout :auth="$page.props.auth" :settings="$page.props.settings">
        ...
        <template #header>
            <div class="flex items-center gap-2">
                <Link :href="route('events.index')" class="text-amber-600 hover:text-amber-700 text-sm">Events</Link>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden">
                    ...
                    <div class="p-8">
                        ...
                        <div v-if="event.is_paid && event.ticket_price" class="border-t border-gray-200 dark:border-gray-700 pt-6 mt-6">
                            ...
                            <div class="text-2xl font-bold text-amber-500 mb-4">{{ event.ticket_currency || 'NGN' }} {{ (event.ticket_price / 100).toFixed(2) }}</div>
                            <div class="flex gap-3">
                                <label v-for="method in ['paystack', 'stripe']" :key="method"
                                    class="flex items-center gap-2 border rounded-lg px-4 py-2 cursor-pointer hover:border-amber-500"
                                    :class="{ 'border-amber-500 bg-amber-50 dark:bg-amber-900/20': ticketForm.payment_method === method }">
                                    <input type="radio" v-model="ticketForm.payment_method" :value="method" class="text-amber-500" />
                                    <span class="text-sm font-medium capitalize">{{ method }}</span>
                                </label>
                            </div>
                            <button @click="buyTicket" :disabled="ticketForm.processing"
                                class="mt-3 bg-amber-500 hover:bg-amber-600 text-white font-medium px-6 py-2 rounded disabled:opacity-50">
