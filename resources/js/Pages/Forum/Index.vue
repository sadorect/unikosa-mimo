    ...
    <AuthLayout :auth="$page.props.auth" :settings="$page.props.settings">
        ...
        <template #header>
            <div class="flex justify-between items-center">
                ...
                <Link :href="route('forum.create')" class="bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium px-4 py-2 rounded">New Post</Link>
            </div>
        </template>
        <div class="py-12">
                        ...
                        <div v-for="post in posts.data" :key="post.id" class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 hover:shadow-md transition">
                            <Link :href="route('forum.show', post.id)" class="block">
                                <div class="flex items-center gap-2 text-sm text-gray-500 mb-2">
                                    ...
                                    <span>&middot;</span>
                                    <span class="text-amber-600">{{ post.group?.name }}</span>
                                </div>
