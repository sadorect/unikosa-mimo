                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <Link v-for="album in albums.data" :key="album.id" :href="route('gallery.show', album)"
                        ...
                        <div class="h-48 bg-gray-200 dark:bg-gray-700 relative">
                            <img v-if="album.media?.[0]?.type === 'image'" :src="album.media[0].path" class="w-full h-full object-cover" />
                            <div v-else-if="album.media?.[0]?.type === 'video'" class="relative w-full h-full">
                                <video :src="album.media[0].path" class="w-full h-full object-cover" preload="metadata" muted></video>
